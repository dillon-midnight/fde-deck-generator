...
- **`/dashboard`** — Dynamic SSR with a Suspense boundary and `loading.tsx` skeleton. The server-side DB query eliminates a client fetch waterfall, and the streamed skeleton gives the browser something to paint immediately, improving LCP on slow connections. After hydration, the `DeckList` client component opens an SSE connection to `/api/decks/dashboard-stream` which multiplexes native Vercel Workflow durable streams from all active runs for real-time slide-count and completion updates — only when active runs exist.
